...
def modify_backbone_to_googlenet(model, num_classes):
    ...
    backbone.out_channels = 1024  # output channels of GoogleNet's last layer before the avgpool
...
def create_detectionmodel(modelname, num_classes=None, trainable_layers=0, ckpt_file=None, fp16=False, device='cuda:0', scale='n'):
    ...
    elif modelname == 'fasterrcnn_googlenet':
        model = modify_backbone_to_googlenet(FasterRCNN, num_classes=num_classes)
        if ckpt_file:
            model = load_checkpoint(model, ckpt_file, fp16)
    else:
        print('Model name not supported')
